...
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   Bc: () => (/* binding */ incrementalExternalId),
/* harmony export */   Ec: () => (/* binding */ resolveIncrementalState),
/* harmony export */   QJ: () => (/* binding */ buildMutationScope)
/* harmony export */ });
/* unused harmony exports buildIncrementalConfigFingerprint, buildLocalDeltaFiles */
...
/* harmony import */ var _utils_exec_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(82224);
...
function buildIncrementalConfigFingerprint(inputs, triggers) {
    const payload = {
    ...
    return (0,node_crypto__WEBPACK_IMPORTED_MODULE_1__.createHash)('sha256').update(stableStringify(payload)).digest('hex').slice(0, 16);
}
/** Return the GitHub Check external_id used to mark reusable incremental baselines. */
function incrementalExternalId(configFingerprint) {
    return configFingerprint
        ? `${INCREMENTAL_EXTERNAL_ID_PREFIX}${configFingerprint}`
        : undefined;
}
/** Build a comment mutation scope from a file list. */
function buildMutationScope(kind, files) {
...
function git(args, cwd) {
    const result = (0,node_child_process__WEBPACK_IMPORTED_MODULE_0__.spawnSync)('git', args, {
        cwd,
        encoding: 'utf-8',
        env: { ...process.env, ..._utils_exec_js__WEBPACK_IMPORTED_MODULE_3__/* .GIT_NON_INTERACTIVE_ENV */ .OO },
...
function buildLocalDeltaFiles(previousHeadSha, headSha, repoPath) {
    const previousExists = gitStatus(['rev-parse', '--verify', `${previousHeadSha}^{commit}`], repoPath);
    if (previousExists !== 0) {
        throw new Error(`Warden incremental baseline ${previousHeadSha} is not available in the checkout. ` +
    ...
    for (const file of files) {
        ...
        file.chunks = (0,_types_index_js__WEBPACK_IMPORTED_MODULE_2__/* .countPatchChunks */ .kV)(file.patch);
    }
    return files;
}
async function findPreviousIncrementalHeadSha(octokit, owner, repo, pullNumber, currentHeadSha, externalId) {
...
async function resolveIncrementalState(options) {
    ...
    const previousHeadSha = externalId
        ? await findPreviousIncrementalHeadSha(options.octokit, options.owner, options.repo, options.pullNumber, headSha, externalId)
        : undefined;
    if (!previousHeadSha) {
        return {
            enabled: true,
...
function parseRequiredBooleanInput(name, fallback) {
...
// UNUSED EXPORTS: IncrementalOutputSchema, TriggerRunResultSchema
...
const ReplaySkillReportSchema = schemas/* object */.Ik({
    ...
    files: schemas/* array */.YO(types/* FileReportSchema */.uP).optional(),
    model: schemas/* string */.Yj().optional(),
});
const IncrementalOutputSchema = schemas/* object */.Ik({
    enabled: schemas/* boolean */.zM(),
    mode: schemas/* enum */.k5(['full', 'delta']),
    previousHeadSha: schemas/* string */.Yj().optional(),
    headSha: schemas/* string */.Yj(),
    configFingerprint: schemas/* string */.Yj().optional(),
    files: schemas/* array */.YO(schemas/* string */.Yj()),
});
...
function incrementalOutput(incremental) {
...
function writeWorkflowFindingsOutput(reports, context, findingObservations, incremental, triggerResults) {
...
function isCommentInScope(comment, scope) {
    if (scope.kind === 'full') {
        return true;
    }
    return scope.files.has(comment.path);
}
function filterCommentsByScope(comments, scope) {
    if (scope.kind === 'full') {
        return comments;
    }
    return comments.filter((comment) => isCommentInScope(comment, scope));
}
function scopeFromReports(fallback, reports) {
    ...
    try {
        ...
        if (context.pullRequest && inputs.incremental) {
            ...
            if (incremental.mode === 'full' && incremental.files.length === 0) {
                const files = await (0,_event_context_js__WEBPACK_IMPORTED_MODULE_4__/* .fetchPullRequestFiles */ .QP)(octokit, context.repository.owner, context.repository.name, context.pullRequest.number);
                incremental = {
                    ...incremental,
                    files,
...
async function dismissPreviousReviewIfResolved(octokit, context, previousReviewInfo, results, canResolveStale, mutationScope, allWardenCommentsResolved, options = {}) {
    ...
    const incrementalAllowsDismissal = mutationScope.kind === 'full' || allWardenCommentsResolved;
    if (context.pullRequest &&
 ...
 */
async function createCompletedCoreCheckForReport(octokit, context, results, reports, shouldFailAction, outputs, overrides = {}, conclusion, externalId) {
    const options = checkOptionsForPullRequest(context);
    if (!options) {
        return;
    }
...
async function runReportMode(octokit, inputs, initResult, repoPath, span) {
    const { context, auxiliaryOptions, matchedTriggers, skippedTriggers, incremental, skipCoreCheck, } = initResult;
...
async function runPRWorkflow(octokit, inputs, eventName, eventPath, repoPath) {
    return _sentry_js__WEBPACK_IMPORTED_MODULE_2__/* .Sentry.startSpan */ .sQ.startSpan({ op: 'workflow.run', name: 'review pull_request' }, async (span) => {
        ...
        const { context, runnerConcurrency, auxiliaryOptions, matchedTriggers, skippedTriggers, incremental, skipCoreCheck, } = initResult;
...
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   QP: () => (/* binding */ fetchPullRequestFiles),
/* harmony export */   eU: () => (/* binding */ buildEventContext)
/* harmony export */ });
...
async function buildEventContext(eventName, eventPayload, repoPath, octokit, options = {}) {
    ...
    if (eventName === 'pull_request' && payload.pull_request) {
        ...
        const files = options.pullRequestFiles ?? (options.skipPullRequestFiles
            ? []
            : await fetchPullRequestFiles(octokit, repository.owner, repository.name, pr.number));
        pullRequest = {
            number: pr.number,
            title: pr.title,
            body: pr.body,
 ...
 */
function findStaleComments(existingComments, allFindings, scope, options = {}) {
    const staleComments = [];
